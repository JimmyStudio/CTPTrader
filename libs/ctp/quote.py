...
class Quote:
	...
	def __init__(self):
		...
		if isWindowsSystem():
			...
			so_name = dlldir+'/ctp_quote.so'
			# print(so_name)
			self.h = cdll.LoadLibrary(so_name)
	...
	def __OnRtnDepthMarketData(self, pDepthMarketData):
		try:
			...
			with open('bug.txt', 'w') as f:
				f.write('**** OnRtnDepthMarketData 崩溃****\n')
				f.write(str(dt.now()) + '\n')
				f.write(str(e) + '\n')
				f.write(str(CThostFtdcDepthMarketDataField) + '\n')
				f.write(str(POINTER(CThostFtdcDepthMarketDataField)) + '\n')
				f.write(str(POINTER(CThostFtdcDepthMarketDataField).from_param(pDepthMarketData)) + '\n')
				f.write('***************')
	...
	def OnRspSubMarketData(self, pSpecificInstrument = CThostFtdcSpecificInstrumentField, pRspInfo = CThostFtdcRspInfoField, nRequestID = int, bIsLast = bool):
		print('QUOTE 行情订阅 OnRspSubMarketData %s'% pSpecificInstrument)
	...
	def OnRtnDepthMarketData(self, pDepthMarketData = CThostFtdcDepthMarketDataField):
		print('QUOTE: 行情tick: %s' % pDepthMarketData)
